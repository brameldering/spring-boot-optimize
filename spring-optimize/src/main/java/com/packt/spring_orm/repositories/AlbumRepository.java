...
public interface AlbumRepository extends JpaRepository<AlbumEntity, Long> {
  ...
  List<PlayerEntity> findByIdMissingPlayers(Long id);

  @Query("SELECT p FROM PlayerEntity p JOIN p.cards c WHERE c.album.id = :id")
  List<PlayerEntity> findByIdPlayers(Long id, Pageable page);

  @Query("SELECT p FROM PlayerEntity p JOIN p.cards c WHERE c.album.id = :id AND p.team.id = :teamId")
  List<PlayerEntity> findByIdAndTeam(Long id, Long teamId);

  List<AlbumEntity> findAllByOwner(UserEntity user);
}
